...
exports.generateExamPrediction = generateExamPrediction;
exports.createOrUpdateExamPrediction = createOrUpdateExamPrediction;
const ExamPrediction_1 = require("../models/ExamPrediction");
const examSignalAggregator_1 = require("./examSignalAggregator");
async function generateExamPrediction(userId, syllabusId) {
    // Aggregate all signals
    const signals = await (0, examSignalAggregator_1.aggregateExamSignals)(userId, syllabusId);
...
function generateFallbackPrediction(signals) {
    ...
    const readinessPercentage = Math.round(completionScore * 0.4 +
        quizScore * 0.4 +
    ...
    const riskTopics = syllabusCompletion.remainingTopics.slice(0, 5).map((topic) => ({
        topic,
        reason: 'Incomplete syllabus coverage',
        riskLevel: 'medium',
    }));
    const strongTopics = quizPerformance.topicScores
        .filter((ts) => ts.score >= 80)
        .slice(0, 5)
        .map((ts) => ts.topic);
    return {
        readinessPercentage,
...
async function createOrUpdateExamPrediction(userId, syllabusId) {
    // Aggregate signals first
    const signals = await (0, examSignalAggregator_1.aggregateExamSignals)(userId, syllabusId);
    // Generate prediction
    const prediction = await generateExamPrediction(userId, syllabusId);
    // Get historical trends
    const existingPredictions = await ExamPrediction_1.ExamPredictionModel.find({ userId, syllabusId })
        .sort({ createdAt: -1 })
        .limit(8)
        .lean();
    ...
    const dataPointsAnalyzed = signals.syllabusCompletion.totalTopics +
        signals.quizPerformance.totalQuizzes +
        signals.revisionCycles.totalRevisions;
    // Create or update prediction
    const existingLatest = await ExamPrediction_1.ExamPredictionModel.findOne({ userId, syllabusId })
        .sort({ createdAt: -1 });
    if (existingLatest) {
        // Update existing
        existingLatest.set({
        ...
        // Create new
        return await ExamPrediction_1.ExamPredictionModel.create({
            ...prediction,
            userId,
            syllabusId,
            historicalTrends,
