...
exports.generateDailyCoachMessage = generateDailyCoachMessage;
exports.saveDailyCoachMessage = saveDailyCoachMessage;
...
async function generateDailyCoachMessage(userId, syllabusId) {
    ...
    const sessions = await FocusSession_1.FocusSessionModel.find({
        userId,
        syllabusId,
        startTime: { $gte: yesterday.toISOString(), $lte: today.toISOString() },
    }).lean();
    // Get yesterday's quizzes
    const quizzes = await QuizResult_1.QuizResultModel.find({
        userId,
        syllabusId,
...
async function saveDailyCoachMessage(userId, syllabusId, coachMessage) {
