...
exports.generateStudyStrategy = generateStudyStrategy;
...
async function generateStudyStrategy(request) {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    // Get user's data
    const plan = await StudyPlan_1.StudyPlanModel.findOne({
        userId: request.userId,
        syllabusId: request.syllabusId
    }).lean();
    const syllabus = await Syllabus_1.SyllabusModel.findById(request.syllabusId).lean();
    const digitalTwin = await DigitalTwin_1.DigitalTwinModel.findOne({
        userId: request.userId,
        syllabusId: request.syllabusId
