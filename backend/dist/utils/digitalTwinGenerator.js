...
exports.createOrUpdateDigitalTwin = createOrUpdateDigitalTwin;
const DigitalTwin_1 = require("../models/DigitalTwin");
const behaviorAggregator_1 = require("./behaviorAggregator");
async function generatePredictiveInsights(userId, syllabusId, aggregatedData) {
    const { averageQuizScore, quizScores, retentionRate, focusScore, completionRate, topicsMastered, topicsInProgress, revisionHistory, cognitiveLoadData, } = aggregatedData;
    // Calculate base predicted score
    let predictedExamScore = Math.round(averageQuizScore * 0.4 +
        retentionRate * 0.25 +
        focusScore * 0.2 +
        completionRate * 0.15);
    // Adjust based on trend
    if (quizScores.length >= 3) {
        const recentTrend = quizScores[quizScores.length - 1] - quizScores[0];
        if (recentTrend > 10) {
    ...
    const dataPoints = quizScores.length +
    ...
    cognitiveLoadData?.forEach((cog) => {
        if (cog.difficulty === 'hard' ||
    ...
    const estimatedPreparationTime = studySpeed > 0 ? Math.ceil(remainingTopics / studySpeed) : 20;
    return {
        predictedExamScore,
        confidenceLevel,
        readinessTrend,
...
async function generateAISummary(userId, syllabusId, digitalTwinData) {
    ...
    try {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 300,
                temperature: 0.7,
                system: 'You are an encouraging educational psychologist who provides insightful, actionable feedback to students.',
                messages: [
                    {
                        role: 'user',
...
async function createOrUpdateDigitalTwin(userId, syllabusId) {
    // Aggregate all user behavior
    const aggregatedData = await (0, behaviorAggregator_1.aggregateUserBehavior)(userId, syllabusId);
    // Calculate learning style
    const learningStyle = (0, behaviorAggregator_1.calculateLearningStyle)(aggregatedData.focusSessions);
    // Calculate performance metrics
    const totalTopics = aggregatedData.topicsMastered + aggregatedData.topicsInProgress;
    const performance = (0, behaviorAggregator_1.calculatePerformanceMetrics)(aggregatedData.quizScores, aggregatedData.focusSessions, aggregatedData.totalSessionsCompleted, totalTopics);
    // Calculate time patterns
    const timePatterns = (0, behaviorAggregator_1.calculateTimePatterns)(aggregatedData.studyByHour, aggregatedData.studyByDayOfWeek, aggregatedData.focusSessions);
    // Generate predictive insights
    const predictiveInsights = await generatePredictiveInsights(userId, syllabusId, aggregatedData);
    ...
    const digitalTwinData = {
        ...
        timePatterns,
        predictiveInsights,
        dataPointsAnalyzed: aggregatedData.focusSessions.length +
    ...
    // Generate AI summary
    const { aiSummary, learningPersonality } = await generateAISummary(userId, syllabusId, digitalTwinData);
    digitalTwinData.aiSummary = aiSummary;
    digitalTwinData.learningPersonality = learningPersonality;
    // Upsert digital twin
    const existingTwin = await DigitalTwin_1.DigitalTwinModel.findOne({ userId, syllabusId }).sort({ createdAt: -1 });
    if (existingTwin) {
        // Update existing
        ...
        // Create new
        return await DigitalTwin_1.DigitalTwinModel.create(digitalTwinData);
    }
}
