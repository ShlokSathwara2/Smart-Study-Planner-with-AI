...
exports.analyzeStudyPatterns = analyzeStudyPatterns;
...
async function analyzeStudyPatterns(userId, syllabusId, minDays = 14) {
    // Get all focus sessions
    const sessions = await FocusSession_1.FocusSessionModel.find({ userId, syllabusId })
        .sort({ startTime: -1 })
        .lean();
    ...
    const sortedSessions = [...recentSessions].sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
    const breaks = [];
    ...
    const dominantLearningStyle = 'visual';
    ...
    const quizzes = await QuizResult_1.QuizResultModel.find({ userId, syllabusId }).sort({ completedAt: 1 }).lean();
    let masterySpeed = 'moderate';
