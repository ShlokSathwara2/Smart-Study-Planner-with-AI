...
const digitalTwinContext_1 = require("../utils/digitalTwinContext");
...
async function generatePlanWithClaude(input) {
    ...
    if (input.userId && input.syllabusId) {
        const twinContext = await (0, digitalTwinContext_1.getDigitalTwinContext)(input.userId, input.syllabusId);
        if (twinContext) {
            digitalTwinPrompt = '\n\n' + (0, digitalTwinContext_1.formatDigitalTwinPrompt)(twinContext);
        }
    }
    const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
            ...
            max_tokens: 1200,
            temperature: 0,
            system: 'You are an expert study planner. Return JSON only with shape: { "sessions": Array<{ "date": "YYYY-MM-DD", "startTime": "HH:mm", "endTime": "HH:mm", "topic": string, "unit"?: string, "estimatedMinutes": number }> }. Ensure sessions do not overlap within the same day, and total per day <= dailyHours.' + digitalTwinPrompt,
            messages: [
                {
                    role: 'user',
                    content: [
...
async function rescheduleWithClaude(input) {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
            ...
            max_tokens: 1400,
            temperature: 0,
            system: 'You are rescheduling a study plan. Return JSON only: { "sessions": Array<{ "date": "YYYY-MM-DD", "startTime": "HH:mm", "endTime": "HH:mm", "topic": string, "unit"?: string, "estimatedMinutes": number }> }. Constraints: dates must be between fromDate and examDate inclusive, no overlaps per day, total per day <= dailyHours.',
            messages: [
                {
                    role: 'user',
                    content: [
                        {
                            ...
                            text: `Reschedule remaining sessions into a recovery plan.\n\nfromDate: ${input.fromDate}\nexamDate: ${input.examDate}\ndailyHours: ${input.dailyHours}\n\nRemaining sessions (keep topics, you may adjust time slots):\n${input.remainingSessions
                                .map((s) => `- ${s.topic} (${s.estimatedMinutes}m) originally ${s.date} ${s.startTime}-${s.endTime}`)
                                .join('\n')}`,
                        },
                    ],
                },
...
router.post('/generate', async (req, res) => {
    try {
        const { userId = 'anonymous', syllabusId, examDate, dailyHours } = req.body;
        if (!syllabusId) {
        ...
        const topics = deps.map((d) => ({ topic: d.topic, dependsOn: d.dependsOn || [] }));
        if (!topics.length) {
        ...
        const sessions = Array.isArray(planJson.sessions) ? planJson.sessions : [];
        // basic validation & internal clash detection
        const clashes = [];
        for (let i = 0; i < sessions.length; i++) {
            const s = sessions[i];
            if (!s?.date || !s?.startTime || !s?.endTime || !s?.topic)
                continue;
        ...
        const saved = await StudyPlan_1.StudyPlanModel.create({
            userId,
            syllabusId,
            examDate,
            dailyHours: Number(dailyHours),
...
router.get('/latest', async (req, res) => {
    try {
        const { userId = 'anonymous', syllabusId } = req.query;
        if (!syllabusId) {
            res.status(400).json({ error: 'syllabusId is required' });
            return;
        }
        const plan = await StudyPlan_1.StudyPlanModel.findOne({ userId, syllabusId }).sort({ createdAt: -1 }).lean();
...
router.patch('/:planId/session', async (req, res) => {
    try {
        const { planId } = req.params;
        const { userId = 'anonymous', sessionIndex, status, actualMinutes } = req.body;
...
router.put('/:planId/sessions', async (req, res) => {
    try {
        const { planId } = req.params;
        const { userId = 'anonymous', sessions } = req.body;
...
router.get('/:planId/progress', async (req, res) => {
    try {
        ...
        const plan = await StudyPlan_1.StudyPlanModel.findOne({ _id: planId, userId }).lean();
        if (!plan) {
            res.status(404).json({ error: 'Plan not found' });
            return;
        }
        ...
        const overdue = sessions.filter((s) => s?.date < today && (s?.status === 'planned' || !s?.status));
        const overdueDays = Array.from(new Set(overdue.map((s) => s.date))).length;
...
router.post('/:planId/reschedule', async (req, res) => {
    try {
        const { planId } = req.params;
        const { userId = 'anonymous', fromDate } = req.body;
        const plan = await StudyPlan_1.StudyPlanModel.findOne({ _id: planId, userId });
        if (!plan) {
            res.status(404).json({ error: 'Plan not found' });
            return;
        ...
        const fixed = sessions.filter((s) => s?.status === 'done' || s?.status === 'skipped');
        ...
        const rescheduled = await rescheduleWithClaude({
            ...
            dailyHours: Number(plan.dailyHours),
            fromDate: startFrom,
            remainingSessions: remaining.map((s) => ({
                date: s.date,
        ...
        const newSessions = Array.isArray(rescheduled.sessions)
