...
router.post('/generate', async (req, res) => {
    try {
        const { userId = 'anonymous', syllabusId, topic, numQuestions = 5 } = req.body;
        if (!syllabusId || !topic) {
        ...
        const estimate = await TopicEstimate_1.TopicEstimateModel.findOne({ userId, syllabusId, topic }).lean();
        ...
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 1500,
                temperature: 0.7,
                system: 'You are creating quiz questions for students. Return JSON only with shape: { "questions": Array<{ "question": string, "options": string[4], "correctAnswer": number (0-3), "explanation": string }>. Questions should test understanding, not just recall. Vary difficulty.',
                messages: [
                    {
                        role: 'user',
                        content: [
...
router.post('/submit', async (req, res) => {
    try {
        const { userId = 'anonymous', syllabusId, planId, topic, questions, attempts } = req.body;
        ...
        // Save quiz result
        const quizResult = await QuizResult_1.QuizResultModel.create({
            userId,
            syllabusId,
            topic,
            questions,
        ...
        if (planId) {
            ...
            const sm2Result = (0, sm2_1.calculateNextReview)(quality);
            ...
            try {
                const plan = await StudyPlan_1.StudyPlanModel.findById(planId);
                if (plan) {
                    plan.sessions.push({
                        date: nextReviewDate,
                        startTime: '20:00',
                    ...
                    plan.sessions.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
                    await plan.save();
                }
            }
            catch (scheduleErr) {
...
router.get('/by-topic/:topic', async (req, res) => {
    try {
        ...
        const quizzes = await QuizResult_1.QuizResultModel.find({ userId, syllabusId, topic })
            .sort({ completedAt: -1 })
            .limit(10)
            .lean();
...
async function updateWeakTopicDetection(userId, syllabusId, topic, latestScore, quizId) {
    // Get all quiz attempts for this topic, sorted by most recent first
    const allQuizzes = await QuizResult_1.QuizResultModel.find({ userId, syllabusId, topic })
        .sort({ completedAt: -1 })
        .lean();
    ...
    const daysSinceLastAttempt = Math.floor((Date.now() - new Date(lastAttempt).getTime()) / (1000 * 60 * 60 * 24));
    ...
    await WeakTopic_1.WeakTopicModel.findOneAndUpdate({ userId, syllabusId, topic }, {
        $set: {
