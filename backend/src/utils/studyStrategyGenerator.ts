...
Respond in JSON format:
{
  ...
  const system = 'You are an expert study planner creating a customized strategy. Always respond in valid JSON.';

  try {
    const content = await callLLM(system, prompt, { maxTokens: 2000, temperature: 0.5, jsonMode: true });
