 ...
 */
export async function runWeeklyDigitalTwinUpdate(): Promise<{
  updated: number;
  skipped: number;
  errors: number;
}> {
  ...
  try {
    ...
    const allTwins = await DigitalTwinModel.aggregate([
      {
        $group: {
          _id: {
            userId: '$userId',
    ...
    for (const twinGroup of allTwins) {
      ...
      if (timeSinceUpdate < ONE_WEEK_MS) {
        console.log(
          `⏭️  Skipping ${userId}/${syllabusId} - only ${Math.round(timeSinceUpdate / (1000 * 60 * 60 * 24))} days since last update`
        );
        stats.skipped++;
        continue;
      }
      ...
      try {
        console.log(
          `📊 Updating digital twin for ${userId}/${syllabusId} (${Math.round(timeSinceUpdate / (1000 * 60 * 60 * 24))} days old)`
        );

        await createOrUpdateDigitalTwin(userId, syllabusId);
        stats.updated++;
        ...
        console.error(
          `❌ Error updating digital twin for ${userId}/${syllabusId}:`,
          error
        );
        stats.errors++;
      }
    ...
    const plansWithoutTwins = await StudyPlanModel.aggregate([
      {
        $group: {
          _id: {
            userId: '$userId',
    ...
    for (const planGroup of plansWithoutTwins) {
      ...
      if (!existingTwin) {
        try {
          ...
          console.error(
            `❌ Error creating digital twin for ${userId}/${syllabusId}:`,
            error
          );
          stats.errors++;
        }
    ...
    console.log(
      `✅ Weekly digital twin update complete: ${stats.updated} updated, ${stats.skipped} skipped, ${stats.errors} errors`
    );
 ...
 */
export function scheduleWeeklyDigitalTwinJob(): void {
