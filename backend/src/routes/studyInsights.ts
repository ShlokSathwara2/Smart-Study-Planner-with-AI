...
import { updateStudyPatterns, getOrCreateDailyMessage } from '../utils/insightManager';
...
router.post('/:syllabusId/analyze', async (req, res): Promise<void> => {
  try {
    const { syllabusId } = req.params;
    const { userId = 'anonymous', minDays = 14 } = req.body as { userId?: string; minDays?: number };

    // Check if we have enough data
    const patterns = await updateStudyPatterns(userId, syllabusId, minDays);
...
router.get('/:syllabusId/daily-message', async (req, res): Promise<void> => {
  try {
    const { syllabusId } = req.params;
    const userId = (req.query.userId as string) || 'anonymous';

    const dailyMessage = await getOrCreateDailyMessage(userId, syllabusId);
...
router.get('/:syllabusId/focus-summary', async (req, res): Promise<void> => {
  try {
    const { syllabusId } = req.params;
    const userId = (req.query.userId as string) || 'anonymous';
